...
int detect::addIndex = 0;
int detect::index = 0;
double detect::info[2]={0};
cv::Mat detect::frame = cv::Mat::zeros(600,600,CV_8UC3);

using namespace cv;
using namespace std;

detect::detect(dnn::Net getnet,VideoCapture getcapture,bool isdetect,bool isGetRect )
    :net(getnet),capture(getcapture),autoControl(false),quitTheThread(false),theHolesNumber(0)       //子对象初始化
{
...
void inline detect::objectPosition(vector<Point2f> &center_p)
{
...
//the big vector
void inline detect::setBottleAndSift(vector<Point2f> &center_p,Mat &frame)
{
    if(tens.size()<10)
    {
...
void inline detect::getConfidenceCentre(vector< vector<Point2f> > tens,Mat &frame)
{
...
double inline detect::distance_(Point2f point1,Point2f point2)
{
    return sqrt(pow(point1.x-point2.x,2)+pow(point1.y-point2.y,2));
}

//sift the points whose thread doesn't meet the requirement
void inline detect:: luminositySift()
{
    int averThread=0;
    int size = 7;
    for(int i = 0;i<theTempCentre.size();i++)
...
//cal theta and distance
void inline  detect::caclDistanceandTheta(vector<Point2f> theLastCentre,Mat &frame,int elemet)
{
    vector<double> theta;
